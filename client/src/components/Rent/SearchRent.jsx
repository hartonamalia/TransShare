...
const SearchRent = ({
  ...
  useEffect(() => {
    if (searchParams) {
      setPickupLocation(searchParams.city || "");
      setPickupDate(searchParams.pickupDate || "");
      setPickupTime(searchParams.pickupTime || "");
      setReturnDate(searchParams.returnDate || "");
      setReturnTime(searchParams.returnTime || "");
      setSelectedCounty(searchParams.auto || "");
      if (searchParams.auto) {
        fetchCities(searchParams.auto);
      }
    }
  }, [searchParams]);
  ...
  const fetchCities = async (countyAuto) => {
    ...
    try {
      const response = await fetch(
        `https://roloca.coldfuse.io/orase/${countyAuto || selectedCounty}`
      );
      const data = await response.json();
      setCities(data);
    } catch (error) {
  ...
  const handleSuggestionClick = (suggestion) => {
    setPickupLocation(suggestion.nume);
    setSuggestions([]);
    setIsMenuOpen(false);
  };
  ...
  return (
    <div className="bg-white p-4 rounded-lg shadow-md max-w-6xl mx-auto absolute left-0 right-0 md:top-[27rem]">
      <form
      ...
      >
        ...
        <div className="flex items-center justify-center gap-3">
          ...
          <button className="mt-5 bg-red-500 hover:bg-red-400 rounded-lg p-1 text-white">
